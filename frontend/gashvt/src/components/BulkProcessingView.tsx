      ...
      {selectedIds.length > 0 && (
        <div className="sticky top-4 z-50 bg-[#0d1117] border border-blue-500 p-4 rounded-2xl flex items-center justify-between shadow-2xl backdrop-blur-md">
          <div className="flex flex-col">
            <span className="text-white font-black text-xs uppercase tracking-tighter">{selectedIds.length} Selected</span>
            <button onClick={() => setSelectedIds([])} className="text-[9px] text-blue-400 font-bold uppercase text-left">Deselect All</button>
          </div>
          <div className="flex gap-2">
            <button onClick={() => handleStatusUpdate(selectedIds, 'FULL')} className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-[9px] font-black uppercase">Mark Full</button>
            <button onClick={() => handleStatusUpdate(selectedIds, 'EMPTY')} className="bg-slate-800 text-white px-4 py-2 rounded-lg text-[9px] font-black uppercase border border-slate-700">Mark Empty</button>
          </div>
        </div>
      )}
      ...
      {Object.entries(batches).map(([batchId, units]) => (
        <div key={batchId} className="space-y-3">
          <div className="flex items-center justify-between px-2">
            ...
            <button onClick={() => toggleBatch(batchId, units.map(u => u.Cylinder_ID))} className="text-blue-500 text-[9px] font-black uppercase">
              Select Batch
            </button>
          </div>
          ...
          <div className="space-y-2">
            {units.map((unit) => {
              ...
              return (
                <div key={unit.Cylinder_ID} className={`grid grid-cols-1 md:grid-cols-4 items-center px-4 py-3 rounded-xl border transition-all gap-3 ${isSelected ? 'bg-blue-500/10 border-blue-500/50' : 'bg-slate-900/50 border-slate-800'}`}>
                  ...
                  <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800 w-fit">
                    <button onClick={() => handleStatusUpdate([unit.Cylinder_ID], 'EMPTY')} className={`px-3 py-1.5 rounded-md text-[8px] font-black uppercase ${unit.Status === 'EMPTY' ? 'bg-slate-800 text-white' : 'text-slate-600'}`}>Empty</button>
                    <button onClick={() => handleStatusUpdate([unit.Cylinder_ID], 'FULL')} className={`px-3 py-1.5 rounded-md text-[8px] font-black uppercase ${unit.Status === 'FULL' ? 'bg-emerald-500/20 text-emerald-500' : 'text-slate-600'}`}>Full</button>
                  </div>
                  ...
                  <div className="text-right">
                    <button onClick={() => handleStatusUpdate([unit.Cylinder_ID], 'Damaged')} className={`flex items-center gap-1 text-[8px] font-black uppercase px-3 py-1.5 rounded-lg border ${unit.Status === 'Damaged' ? 'bg-red-500 text-white' : 'border-slate-800 text-slate-500'}`}>
